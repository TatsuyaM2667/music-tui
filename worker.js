export default {
  async fetch(request, env) {
    ...
    if (rawKey !== null) {
      ...
      for (const key of Array.from(keysToTry)) {
        ...
        if (file) {
          ...
          if (file.range) {
            headers.set(
              "Content-Range",
              `bytes ${file.range.offset}-${file.range.offset + file.range.length - 1}/${file.size}`,
            );
            headers.set("Content-Length", file.range.length.toString());
          } else {
      ...
      return new Response(
        `R2 Key Not Found. Path: ${decodedPath}, Tried: ${JSON.stringify(Array.from(keysToTry))}`,
        { status: 404 },
      );
    }
